...
function useNavbar(props) {
	...
	const navRef = useOnClickOutside((event) => {
		if (open) {
...
const Sidebar = ({ items, activeItemId, setActiveItem, title, excerpt }) => {
	const { navRef, open, toggleOpen } = useNavbar({
		defaultOpen: true,
	});
				...
				<div className={s.sidebar}>
					...
					{items.map((item) => (
						<ThumbButton
							key={item.indexOf}
							src={item.thumbnail}
							active={item.indexOf === activeItemId}
							setActiveItem={() => setActiveItem(item.original)}
							id={item.indexOf}
						/>
					))}
...
// import ThumbButton from './ThumbButton';
//
// function useOnClickOutside(onClickOutside: { (e: any): void }) {
// 	const insideRef = useRef(null);
//
// 	function handleBodyClick(e: { target: any }) {
// 		// @ts-ignore
...
// }
//
// function useNavbar(toggle: { defaultOpen: boolean }) {
// 	const [open, setOpen] = useState(toggle.defaultOpen);
// 	const toggleRef = useOnClickOutside((e) => {
// 		if (open) {
// 			e.preventDefault();
// 			setOpen(false);
// 		}
...
// }
//
// const Sidebar = (items: any, activeItemId: string, setActiveItem: any, title: string, excerpt: string) => {
// 	const { toggleRef, open, toggleOpen } = useNavbar({
// 		defaultOpen: true,
// 	});
//
// 	return (
// 		<>
// 			<section className={open ? s.open : s.closed} ref={toggleRef}>
// 				<button className={s.toggle} onClick={toggleOpen}>
// 					<span />
// 				</button>
// 				<div className={s.sidebar}>
...
// 						</ul>
// 					</div>
// 					{items.map((item: { indexOf: string; thumbnail: string; original: string }) => (
// 						<ThumbButton
// 							key={item.indexOf}
// 							src={item.thumbnail}
// 							active={item.indexOf === activeItemId}
